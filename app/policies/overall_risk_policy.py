...
class RiskPolicy:
    ...
    def to_dict(self):
        """Convert RiskPolicy to dictionary representation."""
        return {
...
class OverallRiskPolicy:
    ...
    def data(self):
        ...
        if self._data is None or self.mt is None or self.mt != modification_timestamp:
            ...
            df = t[0].df
            header_row_index = df.index[(df.iloc[:, 0] == 'Credit Score') & (
                df.iloc[:, 1] == 'Account Status')].tolist()[0]
            df.columns = df.iloc[header_row_index]
            df = df.iloc[header_row_index+1:].reset_index(drop=True)
            temp = []
            for r in df.itertuples():
                try:
                    # camelot sometimes detect as - U+2013 sometimes as a U+002d
                    credit_scores = r[1].split(" ")
                    try:
    ...
    def get_risk_policy(self, credit_score: int, account_status: str):
        """Get all risk policies as list of RiskPolicy."""
        for r in self.data:
            rp: RiskPolicy = r
